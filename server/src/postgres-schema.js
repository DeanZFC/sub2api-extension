...
CREATE TABLE group_entitlement_concurrency_overrides (
  ...
  rule_id BIGINT NOT NULL REFERENCES group_entitlement_rules(id) ON DELETE CASCADE,
...
CREATE INDEX concurrency_overrides_user_idx
  ON group_entitlement_concurrency_overrides(user_id, status, id DESC);
CREATE INDEX concurrency_overrides_rule_idx
...
export const POSTGRES_SCHEMA_MIGRATIONS = Object.freeze({
  ...
  `,
  5: `
    ALTER TABLE synced_users ADD COLUMN IF NOT EXISTS concurrency INTEGER NOT NULL DEFAULT 0;
    ALTER TABLE group_entitlement_rules ADD COLUMN IF NOT EXISTS concurrency_limit INTEGER
      CHECK (concurrency_limit IS NULL OR concurrency_limit >= 0);
    CREATE TABLE IF NOT EXISTS group_entitlement_concurrency_overrides (
      id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      rule_id BIGINT NOT NULL REFERENCES group_entitlement_rules(id) ON DELETE CASCADE,
      group_id BIGINT NOT NULL,
      user_id BIGINT NOT NULL,
